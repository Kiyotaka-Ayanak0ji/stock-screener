...
const AddStockDialog = () => {
  ...
  // Local results from hardcoded list
  ...
  // Merge local + screener results, deduplicate by ticker
  const merged = useMemo(() => {
    ...
    for (const s of screenerResults) {
      if (!seen.has(s.ticker) && !watchlist.includes(s.ticker)) {
        seen.add(s.ticker);
        results.push(s);
      }
    }
  ...
  // Search Screener API with debounce
  useEffect(() => {
