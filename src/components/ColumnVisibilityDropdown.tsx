...
import { Eye, EyeOff, Columns3, Plus, Trash2, Check, X, Crown } from "lucide-react";
...
const ColumnVisibilityDropdown = () => {
  ...
  return (
    <>
                ...
                {customColumns.map(col => {
                  ...
                  return (
                    <div key={col.id} className="flex items-center justify-between px-2 py-1.5 rounded-md text-sm hover:bg-muted/50 group">
                      <button onClick={() => handleToggle(`custom_${col.id}`)} className="flex items-center gap-2 flex-1">
                        {visible ? <Eye className="h-3.5 w-3.5 text-primary" /> : <EyeOff className="h-3.5 w-3.5 text-muted-foreground" />}
                        {col.name}
                      </button>
                      <Button size="icon" variant="ghost" className="h-6 w-6 opacity-0 group-hover:opacity-100 text-muted-foreground hover:text-loss" onClick={() => removeCustomColumn(col.id)}>
            ...
            {showAddForm ? (
              <div className="flex items-center gap-1">
                <Input value={newColName} onChange={e => setNewColName(e.target.value)}
                  ...
                  placeholder="Column name..." className="h-7 text-xs" autoFocus />
      ...
      <PremiumDialog open={premiumOpen} onOpenChange={setPremiumOpen} featureName="Column Visibility" />
    </>
  );
};
