...
import { Share2, Image, FileText, Link, Check, Loader2 } from "lucide-react";
...
const ShareExportButton = ({ tableRef }: ShareExportButtonProps) => {
  ...
  const generateShareLink = async () => {
    if (!user) {
      toast.error("Please sign in to share watchlists");
      return;
    }
    if (stocks.length === 0) {
      toast.error("No stocks to share");
      return;
    }
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-48">
        ...
        <DropdownMenuItem onClick={generateShareLink} className="cursor-pointer gap-2">
          {linkCopied ? (
